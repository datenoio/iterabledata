"""N3 (Notation3) RDF format support using rdflib."""
...
class N3Iterable(BaseFileIterable):
    """N3 RDF format: yields one record per triple (subject, predicate, object)."""
    ...
    def __init__(
        ...
        if options is None:
            options = {}
        if not HAS_RDFLIB:
            raise ImportError("N3 support requires 'rdflib'. Install with: pip install iterabledata[rdf]") from None
