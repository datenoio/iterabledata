"""GraphML format support using NetworkX."""
...
class GraphMLIterable(BaseFileIterable):
    """GraphML format: yields node records then edge records (id/source/target + attributes)."""
    ...
    def __init__(
        ...
        if not HAS_NETWORKX:
            raise ImportError(
                "GraphML support requires 'networkx'. Install with: pip install iterabledata[graph]"
            ) from None
