"""DOT (GraphViz) format support using NetworkX."""
...
class DOTIterable(BaseFileIterable):
    """DOT format: yields node records then edge records (id/source/target + attributes)."""
    ...
    def __init__(
        ...
        if not HAS_NETWORKX:
            raise ImportError(
                "DOT support requires 'networkx'. Install with: pip install iterabledata[graph]"
            ) from None
    ...
    def reset(self) -> None:
        ...
        if self.mode == "r":
            if self.fobj is not None:
                G = nx.nx_pydot.read_dot(self.fobj)
            elif self.filename is not None:
                G = nx.nx_pydot.read_dot(self.filename)
            else:
                G = nx.Graph()
