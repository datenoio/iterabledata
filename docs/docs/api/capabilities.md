...
- **VCF**: `vcf`, `vcard` (vCard contact files; not genomic VCF)
...
3. **Not Yet Implemented**: Some formats like DBF, XML, and XLS have read support but write support hasn't been implemented yet.

4. **Lakehouse tables**: Hudi is read-only here. Delta Lake and Iceberg support writes through their Python libraries.
...
Query the live set with `list_all_capabilities()` (the count changes as writers are added). Common read-only families:

**Statistical data:** ARFF, SAS, SPSS, Stata, RData, RDS, PC-Axis (PX)

**Spreadsheets:** XLS, XLSX, XLSB, ODS (convert to CSV or Parquet to write)

**Network, web, and markup:** PCAP, HTML, RSS/Atom feeds, XML, ZipXML, TAR

**Lakehouse:** Apache Hudi (Delta Lake and Iceberg **do** write)

**Other:** DBF, DXF, MVT, NetCDF, FlatBuffers, genomic VCF/BCF, SAM/BAM/CRAM, and several scientific/geospatial readers

PSV writes (it is a CSV pipe-delimiter subclass). Avro, WARC, MBOX, and vCard (`.vcf`) also write. HDF5 writes **structured** datasets; other dataset layouts raise `WriteNotSupportedError`.

Do not treat vCard (`.vcf`) as genomic Variant Call Format — that is [genomic_vcf](/formats/genomic_vcf).
