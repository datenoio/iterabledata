...
with open_iterable('output.nt', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.nt', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
NT files can be compressed with all supported codecs:
- GZip (`.nt.gz`)
- BZip2 (`.nt.bz2`)
- LZMA (`.nt.xz`)
- LZ4 (`.nt.lz4`)
