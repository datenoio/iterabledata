...
# Basic reading
with open_iterable('data.fbs') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.fbs', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.fbs', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.fbs', mode='w') as dest:
...
FBS files can be compressed with all supported codecs:
- GZip (`.fbs.gz`)
- BZip2 (`.fbs.bz2`)
- LZMA (`.fbs.xz`)
- LZ4 (`.fbs.lz4`)
