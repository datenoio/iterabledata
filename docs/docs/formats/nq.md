...
with open_iterable('output.nq', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.nq', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
NQ files can be compressed with all supported codecs:
- GZip (`.nq.gz`)
- BZip2 (`.nq.bz2`)
- LZMA (`.nq.xz`)
- LZ4 (`.nq.lz4`)
