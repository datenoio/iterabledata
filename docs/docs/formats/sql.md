...
# Basic reading
with open_iterable('data.sql') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.sql', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.sql', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
SQL files can be compressed with all supported codecs:
- GZip (`.sql.gz`)
- BZip2 (`.sql.bz2`)
- LZMA (`.sql.xz`)
- LZ4 (`.sql.lz4`)
