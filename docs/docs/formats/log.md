...
# Basic reading
with open_iterable('data.log') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.log', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.log', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.log', mode='w') as dest:
...
LOG files can be compressed with all supported codecs:
- GZip (`.log.gz`)
- BZip2 (`.log.bz2`)
- LZMA (`.log.xz`)
- LZ4 (`.log.lz4`)
