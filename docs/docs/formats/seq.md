...
# Basic reading
with open_iterable('data.seq') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.seq', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
SEQ files can be compressed with all supported codecs:
- GZip (`.seq.gz`)
- BZip2 (`.seq.bz2`)
- LZMA (`.seq.xz`)
- LZ4 (`.seq.lz4`)
