...
# Basic reading
with open_iterable('data.vtx') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.vtx', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.vtx', mode='w') as dest:
...
VTX files can be compressed with all supported codecs:
- GZip (`.vtx.gz`)
- BZip2 (`.vtx.bz2`)
- LZMA (`.vtx.xz`)
- LZ4 (`.vtx.lz4`)
