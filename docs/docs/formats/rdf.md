...
with open_iterable('output.rdf', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.rdf', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
RDF files can be compressed with all supported codecs:
- GZip (`.rdf.gz`)
- BZip2 (`.rdf.bz2`)
- LZMA (`.rdf.xz`)
- LZ4 (`.rdf.lz4`)
