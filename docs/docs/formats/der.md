...
with open_iterable('output.der', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.der', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
DER files can be compressed with all supported codecs:
- GZip (`.der.gz`)
- BZip2 (`.der.bz2`)
- LZMA (`.der.xz`)
- LZ4 (`.der.lz4`)
