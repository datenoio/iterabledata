...
with open_iterable('output.dta', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.dta', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.dta', mode='w') as dest:
...
DTA files can be compressed with all supported codecs:
- GZip (`.dta.gz`)
- BZip2 (`.dta.bz2`)
- LZMA (`.dta.xz`)
- LZ4 (`.dta.lz4`)
