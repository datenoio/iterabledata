...
with open_iterable('output.shp', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.shp', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
SHP files can be compressed with all supported codecs:
- GZip (`.shp.gz`)
- BZip2 (`.shp.bz2`)
- LZMA (`.shp.xz`)
- LZ4 (`.shp.lz4`)
