...
# Basic reading
with open_iterable('data.mht') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.mht', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.mht', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.mht', mode='w') as dest:
...
MHT files can be compressed with all supported codecs:
- GZip (`.mht.gz`)
- BZip2 (`.mht.bz2`)
- LZMA (`.mht.xz`)
- LZ4 (`.mht.lz4`)
