...
# Basic reading
with open_iterable('data.ckpt') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.ckpt', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
source = open_iterable('data.ckpt')
try:
    for row in source:
        print(row)
finally:
...
try:
    ...
    with open_iterable('data.ckpt', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
CKPT files can be compressed with all supported codecs:
- GZip (`.ckpt.gz`)
- BZip2 (`.ckpt.bz2`)
- LZMA (`.ckpt.xz`)
- LZ4 (`.ckpt.lz4`)
