...
# Basic reading
with open_iterable('data.rio') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.rio', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
RIO files can be compressed with all supported codecs:
- GZip (`.rio.gz`)
- BZip2 (`.rio.bz2`)
- LZMA (`.rio.xz`)
- LZ4 (`.rio.lz4`)
