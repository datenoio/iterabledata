...
try:
    ...
    with open_iterable('data.copy', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.copy', mode='w') as dest:
...
COPY files can be compressed with all supported codecs:
- GZip (`.copy.gz`)
- BZip2 (`.copy.bz2`)
- LZMA (`.copy.xz`)
- LZ4 (`.copy.lz4`)
