...
# Basic reading
with open_iterable('data.rda') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.rda', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.rda', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.rda', mode='w') as dest:
...
RDA files can be compressed with all supported codecs:
- GZip (`.rda.gz`)
- BZip2 (`.rda.bz2`)
- LZMA (`.rda.xz`)
- LZ4 (`.rda.lz4`)
