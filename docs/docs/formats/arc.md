...
# Basic reading
with open_iterable('data.arc') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.arc', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.arc', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.arc', mode='w') as dest:
...
ARC files can be compressed with all supported codecs:
- GZip (`.arc.gz`)
- BZip2 (`.arc.bz2`)
- LZMA (`.arc.xz`)
- LZ4 (`.arc.lz4`)
