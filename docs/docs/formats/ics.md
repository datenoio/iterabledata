...
# Basic reading
with open_iterable('data.ics') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.ics', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.ics', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
ICS files can be compressed with all supported codecs:
- GZip (`.ics.gz`)
- BZip2 (`.ics.bz2`)
- LZMA (`.ics.xz`)
- LZ4 (`.ics.lz4`)
