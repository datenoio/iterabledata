...
with open_iterable('output.gpkg', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.gpkg', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
GPKG files can be compressed with all supported codecs:
- GZip (`.gpkg.gz`)
- BZip2 (`.gpkg.bz2`)
- LZMA (`.gpkg.xz`)
- LZ4 (`.gpkg.lz4`)
