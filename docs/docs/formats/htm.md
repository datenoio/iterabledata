...
# Basic reading
with open_iterable('data.htm') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.htm', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.htm', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.htm', mode='w') as dest:
...
HTM files can be compressed with all supported codecs:
- GZip (`.htm.gz`)
- BZip2 (`.htm.bz2`)
- LZMA (`.htm.xz`)
- LZ4 (`.htm.lz4`)
