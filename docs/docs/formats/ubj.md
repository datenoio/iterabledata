...
with open_iterable('output.ubj', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.ubj', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
UBJ files can be compressed with all supported codecs:
- GZip (`.ubj.gz`)
- BZip2 (`.ubj.bz2`)
- LZMA (`.ubj.xz`)
- LZ4 (`.ubj.lz4`)
