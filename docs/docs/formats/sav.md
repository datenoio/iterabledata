...
# Basic reading
with open_iterable('data.sav') as source:
    for row in source:
        print(row)
# File automatically closed

# Writing (if supported)
with open_iterable('output.sav', mode='w', iterableargs={
    'param1': 'value1'
}) as dest:
...
try:
    ...
    with open_iterable('data.sav', iterableargs={
        'param1': 'value1'
    }) as source:
        for row in source:
            process(row)
...
- **Use bulk operations**: Use `write_bulk()` instead of individual `write()` calls for better performance
  ...
  with open_iterable('output.sav', mode='w') as dest:
...
SAV files can be compressed with all supported codecs:
- GZip (`.sav.gz`)
- BZip2 (`.sav.bz2`)
- LZMA (`.sav.xz`)
- LZ4 (`.sav.lz4`)
